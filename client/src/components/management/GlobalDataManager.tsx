...
import { BankAccount, Investment, InvestmentTransaction, SavingsGoal, SavingsProject, SalaryHistory, UserGlobalData, UserProfile, GeographicLocation, PayrollSlip, ContractType } from '../../core/types';
import { currency, parseAmount, toISODate, today } from '../../lib/utils';
import { GeographicSelector } from '../ui/GeographicSelector';
import { PayrollSlipManager } from './PayrollSlipManager';
...
export function GlobalDataManager({ isOpen, onClose, globalData, onUpdate, years = [], predictedYears = [], onResetYear, onDeletePredictedYear, onResetAll }: GlobalDataManagerProps) {
  ...
  // Payroll Slips state
  const [payrollSlips, setPayrollSlips] = useState<PayrollSlip[]>(globalData.payrollSlips || []);
  const [isPayrollSlipManagerOpen, setIsPayrollSlipManagerOpen] = useState(false);
  ...
  useEffect(() => {
    ...
    setPayrollSlips(globalData.payrollSlips || []);
  }, [globalData]);
  ...
  async function handleSave() {
    try {
      ...
      // Ne pas fermer automatiquement - l'utilisateur peut continuer à modifier
      setTimeout(() => {
        setSaveStatus('idle');
        setSaveMessage('');
      }, 3000);
      ...
      setSaveStatus('error');
      setSaveMessage(`❌ Erreur lors de l'enregistrement : ${error.message || 'Erreur inconnue'}`);
      setTimeout(() => {
        setSaveStatus('idle');
        setSaveMessage('');
      }, 5000);
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="⚙️ Gérer mes données globales" closeable>
      <div className="space-y-6">
        ...
        {activeTab === 'salaryHistory' && (
          <div className="space-y-4">
            ...
            <div className="flex items-center justify-between mb-4 p-4 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-700 rounded-lg">
              <div>
                <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-1">📄 Fiches de paie</h3>
                <p className="text-sm text-gray-600 dark:text-gray-400">
                  Importez vos fiches de paie (PDF) par mois et année. Les salaires seront détectés automatiquement.
                </p>
              </div>
              <button
                onClick={() => setIsPayrollSlipManagerOpen(true)}
      ...
      {/* PayrollSlipManager Modal */}
      <PayrollSlipManager
        isOpen={isPayrollSlipManagerOpen}
        onClose={() => setIsPayrollSlipManagerOpen(false)}
        payrollSlips={payrollSlips}
        onUpdate={async (slips) => {
          setPayrollSlips(slips);
          // Sauvegarder automatiquement dans globalData
          await onUpdate({
            ...globalData,
            payrollSlips: slips,
          });
        }}
        currentYear={typeof currentYear === 'number' ? currentYear : today.getFullYear()}
        onUseForIncome={(netSalary, contractType, employer) => {
          // Cette fonctionnalité sera implémentée pour mettre à jour automatiquement les revenus
          alert(`💡 Fonctionnalité en développement : Le salaire de ${currency(netSalary)} de ${employer} (${contractType}) sera automatiquement utilisé pour mettre à jour vos revenus.`);
        }}
      />
    </Modal>
  );
