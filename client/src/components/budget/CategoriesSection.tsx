...
import { currency, defaultCategories, parseAmount } from '../../lib/utils';
    ...
    <section className="bg-white dark:bg-gray-800 rounded-2xl shadow p-4 md:p-6 space-y-4 border border-gray-200 dark:border-gray-700">
      <div className="flex items-center justify-between">
        ...
        >
          ...
          <span>Catégories variables & cibles</span>
        </button>
        {isSectionExpanded && (
          <button
            ...
            >
              ...
              <div className="space-y-2">
                {!isAnnualSimpleMode ? (
                  ...
                  // Budget annuel simple avec indication claire
                  <div className="space-y-2">
                    <div className="flex items-center gap-2">
                      <label className="text-xs text-slate-600 dark:text-gray-400 whitespace-nowrap">Budget annuel:</label>
                      <input
                        type="text"
                        className="flex-1 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2 text-sm text-gray-900 dark:text-white placeholder-gray-500 dark:placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400"
                        ...
                        onBlur={(e) => {
                          ...
                          if (val > 0) {
                            onUpsertCategory(c.id, { target: val });
                          }
                        }}
                      />
                      <span className="text-xs font-medium text-slate-600 dark:text-gray-400 px-2 py-2 bg-gray-100 dark:bg-gray-700 rounded-lg whitespace-nowrap">
                        € par an
                      </span>
                    </div>
                    {c.target > 0 && (
                      <div className="text-xs text-slate-500 dark:text-gray-500 bg-gray-50 dark:bg-gray-800/50 rounded px-2 py-1">
                        Soit ~{currency(Math.round(c.target / 12 * 100) / 100)} par mois
                      </div>
                    )}
                    <button
                      className="w-full px-3 py-2 text-xs font-medium bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 rounded-lg hover:bg-blue-200 dark:hover:bg-blue-900/50 transition-colors border border-blue-200 dark:border-blue-800"
                      onClick={() => {
                      ...
                      title="Basculer vers un budget mensuel détaillé (recommandé)"
                    >
                      📅 Basculer vers budget mensuel
                    </button>
                  </div>
                )}
              </div>
