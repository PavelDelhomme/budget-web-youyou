import { Category, Subscription, YearData, UserGlobalData } from '../core/types';
...
export function generatePredictions(
  ...
  // Need at least 1 year with meaningful data
  const hasEnoughData = historicalYears.some(({ data }) => {
    const hasExpenses = data.expenses && data.expenses.length > 0;
    const hasCategoriesWithTargets = data.categories && data.categories.some(c => c.target > 0);
  ...
  if (historicalYears.length === 0 || !hasEnoughData) {
    return [];
  }
